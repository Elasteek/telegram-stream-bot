...
def send_message_to_admin(user_id, first_name, last_name, username, text):
    ...
    print(f"Отправка сообщения: {data}")  # Печать в консоль для 
наглядности
...
ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"Привет, {user.first_name}! Я бот для 
отправки сообщений администраторам.")
...
Администраторы получили его.")
    else:
        await update.message.reply_text("Извините, произошла ошибка при 
отправке сообщения. Попробуйте позже.")

async def main():
    # Инициализируем бота
    print(f"Инициализация бота с токеном: 
{BOT_TOKEN[:5]}...{BOT_TOKEN[-5:]}")
